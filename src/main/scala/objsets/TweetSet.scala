...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def maxRetweets(tweet: Tweet): Tweet =
    if (this.retweets >= tweet.retweets) this
    else tweet
}
...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList =
    if (isEmpty) Nil
      ...
      new Cons(mostRetweetedVal, remove(mostRetweetedVal).descendingByRetweet)
    }
...
//    (left union (right union that)) incl elem
  ...
  def mostRetweeted: Tweet = {
    ...
    lazy val rightMostRetweeter = right.mostRetweeted
    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) elem.maxRetweets(rightMostRetweeter)
    else if (right.isEmpty) elem.maxRetweets(leftMostRetweeted)
    else elem.maxRetweets(rightMostRetweeter.maxRetweets(leftMostRetweeted))
  }
